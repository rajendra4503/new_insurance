...
if($_POST['type'] == 'update' && $_POST['last_name'] != '' && $_POST['first_name'] != '' && $_POST['mobile_number'] != ''){
  ...
  $query = "UPDATE `employee_details` SET `First_Name` = '$first_name', `Last_Name` = '$last_name', `Date_Of_Birth` = '$date_birth', `Sex` = '$sex', `Mobile` = '$mobile_number', `Email_Id` = '$email', `Address_Line1` = '$address_1', `Adress_Line2` = '$address_2', `City` = '$city', `State` = '$state', `Postal_Code` = '$postal_code', `Country` = '$country', `Updated_Date` = now(),`Updated_By` = '$CostId2' WHERE `Customer_Code` ='$CostId2' AND `plan_sequence_number` = $seqno AND `Employee_Number` ='$EmpNo'";
...
if( $_POST['type'] == 'insert' && $_POST['last_name'] != '' && $_POST['first_name'] != '' && $_POST['mobile_number'] != ''){
  ...
  $PolicyID = $Cost_Id.$seqno.$employee_number;

  $query = "INSERT INTO `employee_details` (`Customer_Code`, `Employee_Number`,`PolicyID`,`First_Name`, `Last_Name`, `Date_Of_Birth`, `Sex`, `Mobile`, `Email_Id`, `Address_Line1`, `Adress_Line2`, `City`, `State`, `Postal_Code`, `Country`, `Created_Date`, `Created_By`,`Effective_Date`) VALUES ('$Cost_Id', '$employee_number','$PolicyID','$first_name','$last_name', '$date_birth', '$sex', '$mobile_number', '$email', ' $address_1', '$address_2', '$city', '$state', '$postal_code','$country',now(),'$Cost_Id',now())";
    ...
    $date_birth  =  date('Y-m-d',strtotime($_POST['dep_date_birth'][$i]));
      ...
      $emp_query = "INSERT INTO `employee_dependent` (`Emp_Code`,`Employee_Number`,`Last_Name`, `First_Name`, `Gender`, `Date_Birth`, `Created_Date`,`Created_By`) VALUES ('$emp_id','$employee_number','$last_name','$first_name','$gender','$date_birth',now(),'$Cost_Id')";
...
<html lang="en" class="no-js">
    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 paddingrl0">
        <div id="plantitle">
          <span>Employee Details</span>  &nbsp; &nbsp; &nbsp; &nbsp;<span>Customer ID :  &nbsp; &nbsp;<?php echo $CostId ;?></span>
        </div>
    </div>

  <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 paddingrl0" style="margin-top:5px;" id="mainplanlistdiv">

        <!-- <a href="customer_add_employee.php?CostId=<?php echo $CostId;?>" style="margin-left: 16px;"><button type="button" class="btn btn-success btn-sm"><i class="fa fa-plus"></i> Add New Employee</button>
        </a> -->

        <a style="margin-left: 16px;" href="#addCode" data-toggle="modal"><button type="button" class="btn btn-success btn-sm"><i class="fa fa-plus"></i> Upload Excel</button>
        </a>
          ...
          <div class="table-responsive">
            ...
            <tbody>
              <?php
                  ...
                  $query = "SELECT * FROM employee_details WHERE  Customer_Code = '$CostId' AND plan_sequence_number=$seqno";
               ...
               <tr>
                  ...
                  <td><a href="customer_edit_employee.php?EmpNo=<?php echo $EmpNo ;?>&CostId=<?php echo $CostId;?>&PlanSeq=<?php echo $seqno;?>" class="edit" data-toggle="modal"><i class="material-icons" data-toggle="tooltip" title="Edit">&#xE254;</i></a></td>
              </tr>
             <?php 
                 $i++;
                } 
