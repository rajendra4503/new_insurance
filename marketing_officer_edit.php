...
if (!empty($_GET['id']) != '') {
    ...
    $query = mysql_query("SELECT * FROM `marketing_officer_details` WHERE Officer_ID = $id AND Created_By = $logged_userid");

    if(mysql_num_rows($query) > 0){
       $officer = mysql_fetch_assoc($query);
    }else{
       header('Location: marketing_officer_details.php');
    }
...
<html lang="en" class="no-js">
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1.0"> 
		<title>eTPA - Marketing Officer</title>
...
<?php
   ...
   if($_POST['last_name'] != '' && $_POST['first_name'] != '' && $_POST['mobile_number'] != '' && $_POST['officer_id'] != ''){
      ...
      $query = "UPDATE `marketing_officer_details` SET 
      ...
      `Updated_Date`='now()',
      `Updated_By`='$logged_userid' WHERE `Officer_ID` = $officer_id";
...
<form action="<?php echo $_SERVER['PHP_SELF'];?>"  name="basicform" id="basicform" method="post" enctype="multipart/form-data">
  ...
  <div class="col-md-6 col-sm-6">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control input-sm" id="mobile_number" name="mobile_number" value="<?php  echo $officer['Mobile'];?>" required>
        </div>
        ...
        <div class="form-group">
            <label for="gender">Phone Number</label>
             <span class="help-block">Enter number with area code,Country code not necessary</span>
            <input type="text" class="form-control input-sm" id="phone_number" name="phone_number" value="<?php echo $officer['Phone'];?>">
        </div>

        <div class="form-group">
            <label for="gender">Fax Number</label>
             <span class="help-block">Enter number with area code,Country code not necessary</span>
            <input type="text" class="form-control input-sm" id="Fax" name="Fax" value="<?php echo $officer['Fax'];?>">
        </div>
  ...
  <div class="col-md-6 col-sm-6">
      ...
      <div class="form-group">
          ...
          <input type="text" class="form-control input-sm" id="postal_code" name="postal_code" value="<?php echo $officer['Postal_Code'];?>" required>
      </div>
...
<script type="text/javascript">
  ...
  jQuery().ready(function() {
	...
	$(document).ready(function() {
        ...
        $('#plapiper_pagename').html("Marketing Management");
        var windowheight = h;
