...
<html lang="en" class="no-js">
    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 paddingrl0" style="margin-top:5px;" id="mainplanlistdiv">
      ...
      <div class="col-sm-6"> 
        <form class="form-inline" action="<?php echo $_SERVER['PHP_SELF'];?>" method="post">
         ...
         <input name="Hospital_Name" class="form-control" type="text" aria-label="Search" style="width: 300px;" required>
            <button type="submit" name="HospitalNameSubmit" class="btn btn-primary" type="submit">Search</button>
        </form>
      </div>
      ...
      <?php
            ...
            $query = "SELECT P.Patient_Name,P.Patient_Last_Name,P.Policy_No,P.Date_of_Admission,H.Hospital_Name,P.Claim_ID,H.Declaration_Date,P.Status FROM patient_details AS P JOIN hospital_details AS H ON P.Claim_ID = H.Claim_ID";

            if(isset($_POST['ClaimID']) !=''){

               $query .= " WHERE P.Claim_ID='".$_POST['ClaimID']."'";
               ...
               $query .= " WHERE P.Policy_No='".$_POST['PatientInsId']."'";

            }elseif (!empty($_POST['Hospital_Name']) !='') {

               $Hospital_Name = $_POST['Hospital_Name'];

               $query .= " WHERE H.Hospital_Name LIKE '%".$Hospital_Name."%'";
                ...
                $query .= " WHERE P.Date_of_Admission BETWEEN  '".$from."' AND '".$to ."'";
            ...
            if(isset($_POST['ClaimID']) !=''){
              echo "<div style='text-align: center;font-size: 20px;font-weight: bold;'>Claim ID : ".$_POST['ClaimID']." </div>";
               ...
               <div style='text-align: center;font-size: 20px;font-weight: bold;'>Status : <?php  if($_POST['status'] == 1){
                   echo 'Verified';
...
<span style="color: #004F35;font-weight: bold;font-size: 17px;" class="verify">Processed</span>
  ...
  $("#submission_from").keypress(function(event) {event.preventDefault();});

  $("#submission_to").keypress(function(event) {event.preventDefault();});
  ...
  $("#admission_to").keypress(function(event) {event.preventDefault();});
	...
	$(document).ready(function() {
        ...
        $('.reviewuser').click(function(){
           ...
           window.location.href = "claim_view.php?claimID="+claimID;
        });
