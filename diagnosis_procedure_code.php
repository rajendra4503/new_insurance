...
<html lang="en" class="no-js">
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <form name="codeeditform" id="codeeditform" method="post">
          <div class="modal-header">            
            <h4 class="modal-title">Edit Diagnosis Procedure Code</h4>
            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
          </div>
          <div class="modal-body">          
            <div class="form-group">
  ...
  $("#addCodeForm").validate({
        ...
        messages:{
         ...
         submitHandler: function(form,e) {
            ...
            $.ajax({
                type: 'POST',
                url: 'ajax/icd_code_procedure_insert.php',
                dataType: "html",
                data: $('form').serialize(),
                success: function(result) {
                   var data = $.parseJSON(result);
                    if(data.status == 'ok'){
                        ...
                        $('#addCodeForm')[0].reset();
                        bootbox.alert({
                         message: "Diagnosis Procedure Code Added Successfully.",
                         size: 'large'
                        });
 ...
 function editFunction(id){
    ...
    $.ajax({
        type: 'POST',
        url: 'ajax/icd_code_procedure_edit.php',
        data    : dataString,
        datatype: 'json',
        cache: false,
   ...
   $('#update_code').click(function(e){
      ...
      $.ajax({
          type: 'POST',
          url: 'ajax/icd_code_procedure_update.php',
          data : dataString,
          datatype:'json',
          cache: false,
          success: function (data) {
              var data1 = $.parseJSON(data);
                 if(data1.status == 'ok'){
                     ...
                     $('#editCode').modal('hide');
                     bootbox.alert({
                         message: "ICD 10 PCS Code Updated Successfully.",
                         size: 'small'
                      });
                  }else{
                      bootbox.alert({
...
$(document).ready(function() {
    ...
    $('#example').DataTable({
        'ajax' : 'ajax/diagnosis_code_procedure_ajax.php',
        'select': true,
        'ordering': false,
        'info': true,
        'scrollY': 350,
        ...
        "searching": true,
        'language': {
          searchPlaceholder: "Search  By ICD 10 PCS Code Or Description",
          search: "_INPUT_"
        },
	...
	$(document).ready(function() {
        ...
        var currentpage = "diagnosis_procedure_code";
        $('#'+currentpage).addClass('active');
        $('#plapiper_pagename').html("Diagnosis Code");
